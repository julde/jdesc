...
makeTeleGui{|tele,id|
		var w, r, startButton, sliders,boxes;
		var cmdPeriodFunc;
		...
		startButton.action = {|view|
				if (view.value == 1) {
					// start sound
					//r.sendMsg("/s_new",name,id);
					r.sendBundle(r.latency, ["/s_new", name, id, 0, 1] ++ getSliderValues.value);
					idBox.enabled_(false);
				};
				if (view.value == 0) {
					if (this.hasGate) {
						// set gate to zero to cause envelope to release
						r.sendMsg("/n_set", id, "gate", 0);
					}{
						r.sendMsg("/n_free", id);
					};
					idBox.enabled_(true);
		...
		var ctlname, ctlname2, capname, spec;
			...
			if (spec.notNil) {
				boxes[i] = gui.ezNumber.new(w,85 @ 15, capname, spec, 
					{ |ez| 
					 r.sendMsg("/n_set", id, ctlname, ez.value);
